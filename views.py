...
#2.2 ChIP analysis
def Single_ChIP_analysis_async(request):
    if request.method == 'GET':
        taskstate = request.GET.get("success")
        if taskstate == "1":
            # async task completed
            ...
            if taskid:
                ...
                task_return = task_result.get()
                if task_return:
                    # Task completed, return [sample_id, tssRegion, species, ignore_1st_exon, ignore_1st_intron, ignore_downstream,ignore_promoter_subcategory, output_folder_name]
                    context = {
                        ...
                        'output_folder_name' : task_return[7]
                    }
                    return render(request, 'ChIP/ChIP_search_main.html', context)

                # task_return == None means no database queue hit, probably due to faulty input.
                return render(request, 'ChIP/ChIP_search_main.html',
                              {'error_message': 'Invalid entry. Please check gene nomenclature and submit the query '
                                                'again.'})
        ...
        query_list = request.POST.get('main_query').split(';')  # Original query data
        query_species = request.POST.get('query_species')  # Species to query
        query_tss = request.POST.get('tssRegion')  # Tss Region setting
        ignore_1st_exon = request.POST.get('ignore_1st_exon')  # True value
        ...
        # Deploy task
        async_result = celery_tasks.ChIP_query_handler.delay(
            query_list, query_tss, query_species,ignore_1st_exon, ignore_1st_intron, ignore_downstream,ignore_promoter_subcategory
        )
        # Return page containing task id, awaiting frontend recognition
        return render(request, 'miRNA/miRNA_search_main.html', {'task_id': async_result.task_id})
