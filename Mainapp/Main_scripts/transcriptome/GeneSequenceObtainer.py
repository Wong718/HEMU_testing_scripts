...
def gene_sequence_query(gene_id_list, query_species, query_type, write_fasta=False):
    """

    :param gene_id_list:
    :param query_species:
    :param query_type:
    :param write_fasta: Boolean value (True/False), write query result to a single fasta file, returning its unique
        identifier (file in static/Temp_R_genefam/[identifier].fasta)
    :return:
    """
    ...
    if query_type == "gene":
        target_fasta = "global-static/annot-sequences/%s.gene.fasta" % query_species
    elif query_type == "transcript":
        target_fasta = "global-static/annot-sequences/%s.transcripts.fasta" % query_species
    elif query_type == "protein":
        target_fasta = "global-static/annot-sequences/%s.proteins.fasta" % query_species
    else:
